...
dif_plot <- ggplot(dif, aes(x= min.speed, y = change.in.production.rate, group = 1)) + geom_point(aes(color = min.speed)) + scale_color_viridis() + xlab('slow codon speed') + ylab('change in rate from generations 1 to 50') + theme(legend.position = 'none') + ggtitle('Overall changes in production rate') + labs(color = 'slow codon speed') + theme_bw()
plot(dif_plot)
