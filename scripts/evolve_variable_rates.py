...
def main():
    ...
    parser.add_argument(
         ...
         required=True,
         type=int,
         help="Set the number of generations to evolve for, required",
         )
    parser.add_argument(
         '-r',
         action='store',
    ...
    while i <= max_generations:
        ...
        ptsim.simulate(gen, 'popA', popA_weights, rates)
        popA_fitness = Decimal(fit_eval.main("../data/generation_" + str(gen)+ '_' + 'popA_' + str(rates[0]) + '_' + str(rates[1]) + '_counts.tsv', 'proteinP')).quantize(Decimal('.001'), rounding = ROUND_HALF_EVEN)
        ...
        ptsim.simulate(gen, 'popB',popB_weights, rates)
        popB_fitness = Decimal(fit_eval.main("../data/generation_" + str(gen)+ '_' + 'popB_' +  str(rates[0]) + '_' + str(rates[1]) + '_counts.tsv', 'proteinP')).quantize(Decimal('.001'), rounding = ROUND_HALF_EVEN)
