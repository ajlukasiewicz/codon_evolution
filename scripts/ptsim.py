...
def simulate(gen, pop, transcript_weights, rates):
    transcript_weights = np.repeat(transcript_weights, 3)
    transcript_length = len(transcript_weights)
    execute("../data/generation_" + str(gen) + '_' + str(pop) + '_' + str(rates[0]) + '_' + str(rates[1]), transcript_weights, transcript_length)
